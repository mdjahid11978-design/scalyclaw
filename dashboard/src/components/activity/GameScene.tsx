...
export function GameScene({
  ...
  const prevXRef = useRef(STATION_MAP.home.x);
  const walkTimerRef = useRef<ReturnType<typeof setTimeout>>();
  const idleTimerRef = useRef<ReturnType<typeof setTimeout>>();

  const targetX = STATION_MAP[target]?.x ?? STATION_MAP.home.x;
  const walkDist = Math.abs(targetX - prevXRef.current);
  const walkMs = walkDist > 1 ? Math.max(400, walkDist * 16) : 0;
  ...
  // Movement on target change
  useEffect(() => {
    const newX = STATION_MAP[target]?.x ?? STATION_MAP.home.x;
    const prev = prevXRef.current;
    prevXRef.current = newX;

    if (Math.abs(newX - prev) < 1) {
      setIsWalking(false);
      return;
    }

    setFacing(newX > prev ? 'right' : 'left');
    setIsWalking(true);

    clearTimeout(walkTimerRef.current);
    const dur = Math.max(400, Math.abs(newX - prev) * 16);
    walkTimerRef.current = setTimeout(() => setIsWalking(false), dur);
    return () => clearTimeout(walkTimerRef.current);
  }, [target]);

  // Idle random facing
  useEffect(() => {
    if (isWalking || sleeping || working) {
      clearTimeout(idleTimerRef.current);
      return;
    }
    const flip = () => {
      setFacing((f) => (f === 'left' ? 'right' : 'left'));
      idleTimerRef.current = setTimeout(flip, 6000 + Math.random() * 8000);
    };
    idleTimerRef.current = setTimeout(flip, 6000 + Math.random() * 8000);
    return () => clearTimeout(idleTimerRef.current);
  }, [isWalking, sleeping, working]);

  const dragonAnim = isWalking
    ? 'animate-walk-bounce'
    : sleeping
      ? ''
      : working
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(180deg, #020806 0%, #030303 55%, #0a120a 100%)',
      }}
    >
      {/* Stars */}
      {stars.map((s, i) => (
        <div
          ...
          style={{
            ...
            width: s.s,
            height: s.s,
            animation: `twinkle ${s.t}s ease-in-out infinite`,
        ...
        fireflies.map((f, i) => (
          <div
            ...
            style={{
              left: `${f.x}%`,
              top: `${f.y}%`,
              animation: `firefly ${f.t}s ease-in-out infinite`,
      ...
      <div
        ...
        style={{
          ...
          background:
            'linear-gradient(90deg, transparent 1%, #10b98125 15%, #10b98118 85%, transparent 99%)',
        }}
      />
      ...
      <div
        ...
        style={{
          left: `${STATIONS[0].x - 1}%`,
          right: `${100 - STATIONS[STATIONS.length - 1].x - 1}%`,
          bottom: `${GROUND_BOTTOM - 0.5}%`,
          background:
            'repeating-linear-gradient(90deg, #10b98112 0px, #10b98112 3px, transparent 3px, transparent 7px)',
        }}
      />
      ...
      <div
        ...
        style={{
          ...
          background: 'linear-gradient(180deg, #080e08 0%, #050805 100%)',
        }}
      />
      ...
      {/* Stations */}
      {STATIONS.map((s) => {
        const here = target === s.id && !isWalking;
        ...
        return (
          ...
          >
            ...
            {note > 0 && (
              <div
                className="absolute -right-2 -top-2 z-30 flex h-4 w-4 items-center justify-center rounded-full text-[7px] font-bold text-white"
            ...
            <div
              ...
              style={{
                borderColor: here ? `${s.color}80` : 'rgba(255,255,255,0.06)',
                background: here ? `${s.color}18` : 'rgba(255,255,255,0.02)',
                boxShadow: here
                  ? `0 0 20px ${s.color}30, 0 0 6px ${s.color}15`
                  : 'none',
              }}
            >
            ...
            <span
              ...
              style={{ color: here ? s.color : '#404040' }}
            >
              {s.label}
            </span>
              ...
              <div className="mt-0.5 flex gap-0.5">
                {channels.slice(0, 8).map((ch, ci) => (
                  <div
                    ...
                    style={{
                      background: ch.enabled ? '#3b82f6' : '#262626',
                    }}
                    title={ch.id}
                  />
                ))}
      ...
      <div
        ...
        style={{
          ...
          bottom: `${GROUND_BOTTOM - 1}%`,
          transform: 'translateX(-50%)',
          transition: walkMs > 0 ? `left ${walkMs}ms ease-in-out` : 'none',
        }}
      >
        {/* Speech bubble */}
        {speech && !sleeping && (
        ...
        {sleeping && (
          <div className="absolute -right-2 -top-8 z-20 flex flex-col items-start gap-0.5">
            {['z', 'z', 'Z'].map((ch, i) => (
              <span
                key={i}
                className="font-mono font-bold text-emerald-400/40"
                style={{
                  fontSize: 8 + i * 3,
        ...
        {/* Working sparkles at station */}
        {working && !isWalking && !sleeping && (
          <>
            {[0, 1, 2, 3, 4].map((i) => (
              <div
                key={i}
                ...
                style={{
                  left: `${15 + Math.cos(i * 1.26) * 55}%`,
                  top: `${5 + Math.sin(i * 1.26) * 35}%`,
        ...
        {/* Ambient glow under dragon */}
        <div
          className="absolute -bottom-2 left-1/2 -translate-x-1/2 rounded-full blur-xl transition-opacity duration-1000"
          style={{
            width: 80,
            height: 20,
            background: sleeping ? '#10b98108' : '#10b98118',
          }}
        />
      ...
      {/* Scene dimming when sleeping */}
      {sleeping && (
        <div className="pointer-events-none absolute inset-0 bg-black/30 transition-opacity duration-2000" />
      )}
    </div>
  );
}
